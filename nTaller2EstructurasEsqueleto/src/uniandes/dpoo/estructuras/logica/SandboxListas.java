...
public class SandboxListas
{
    ...
    public void eliminarEntero( int valor )
    {
    	...
    	while(iterador.hasNext()) {
    		Integer siguiente = iterador.next();
    		if (siguiente.equals(valor)) {
    			
    //El iterador tiene su propio metodo remove
    ...
    public void reiniciarArregloCadenas( List<Object> objetos )
    {
    	...
    	while(iterador.hasNext()) {
    		...
    		String siguiente = (iterador.next()).toString();
    		listaCadenas.add(siguiente);
    ...
    public void volverPositivos( )
    {
    	...
    	for(int i = 0; i<listaEnteros.size(); i++) {
    		if (listaEnteros.get(i) < 0) {
    			listaEnteros.add(i, listaEnteros.get(i)*-1);
    ...
    public int contarApariciones( String cadena )
    {
    	...
    	while(iterador.hasNext()) {
    		String cadena_act = iterador.next();
    		if (cadena_act.toLowerCase().equals(cadena.toLowerCase())) {
    			contador++;
    		}
    	}
    ...
    public int contarEnterosRepetidos( )
    {
    	...
    	for(int i = 0; i<listaEnteros.size(); i++) {
    		...
    		int contador = 0;
    		int pivot = listaEnteros.get(i);
    		
    		for(int j = 0; j<listaEnteros.size(); j++) {
    			
    			if (pivot == listaEnteros.get(j)) {
    				contador++;
    			}
    		...
    		if (contador > 1 && (lista_aux.contains(pivot) == false)) {
    			lista_aux.add(pivot);	
    		}	
    	}
